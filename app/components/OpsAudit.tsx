...
// ─── Dummy data ─────────────────────────────────────────────────────────────
const DUMMY_CITIES = ["Hyderabad", "Delhi", "Gurugram", "Noida", "Faridabad"];
const DUMMY_STATUSES = ["Accepted", "Rejected", "Pending"];
const QC_QUESTIONS = [
  "All valid RTO documents available (License/RC/Insurance/PUC/Fitness/Tax/Permit)",
  "Vehicle exterior clean and undamaged",
  "Tyres in good condition including stepney",
  "All lights and indicators working",
  "Interior clean and seats in good condition",
  "First-aid kit and fire extinguisher present",
];

const pad = (n: number) => String(n).padStart(2, "0");

const DUMMY_RECORDS: AuditRow[] = Array.from({ length: 42 }, (_, index) => {
  const city = DUMMY_CITIES[index % DUMMY_CITIES.length];
  ...
  const ok = (seed: number) => ((index + seed) % 5 === 0 ? 0 : 1);

  return {
    audit_id: String(2000 + index),
    audit_date: dateStr,
    driver_id: String(10200000 + index),
    vehicle_number: `TG05OA${String(6000 + index).padStart(4, "0")}`,
    city,
    ...
    interior: ok(4),
    auditor: `Ops User ${(index % 6) + 1}`,
    final_status: status,
    questions: QC_QUESTIONS.map((q, qi) => ({
      question: q,
      answer: (index + qi) % 4 === 0 ? "No" : "Yes",
    })),
  };
});
...
export default function OpsAudit() {
  ...
  useEffect(() => {
    ...
    const start = dateRange.fromDate ? dayjs(dateRange.fromDate) : null;
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Ops Audit");
    XLSX.writeFile(wb, `Ops_Audit_${dayjs().format("YYYY-MM-DD")}.xlsx`);
  };
  ...
  return (
    <Box sx={{ display: "flex", justifyContent: "center" }}>
      <Paper elevation={0} sx={{ p: 2.5, width: "100%", maxWidth: 1360 }}>
          ...
          >
            ...
            >
              <FormControl size="small" sx={{ width: { xs: "100%", sm: 160 } }}>
                <InputLabel id="ops-city-label">City</InputLabel>
                <Select
                  labelId="ops-city-label"
                  id="ops-city"
        ...
        >
          <Table stickyHeader size="small" aria-label="ops audit table">
            {hasTableData && (
              <TableHead>
                <TableRow>
